...
func LoadConfig() (*Config, error) {
	...
	var cfg Config
	// Accept both historic {"clientId":...} and server-style {"id":...} files.
	var raw map[string]json.RawMessage
	if err := json.Unmarshal(data, &raw); err != nil {
		return nil, err
	}
	if v, ok := raw["clientId"]; ok {
		_ = json.Unmarshal(v, &cfg.ClientID)
	} else if v, ok := raw["id"]; ok {
		_ = json.Unmarshal(v, &cfg.ClientID)
	}
	if v, ok := raw["subdomain"]; ok {
		_ = json.Unmarshal(v, &cfg.Subdomain)
	}
	if v, ok := raw["customDomain"]; ok {
		_ = json.Unmarshal(v, &cfg.CustomDomain)
	}
	if v, ok := raw["ownerEmail"]; ok {
		_ = json.Unmarshal(v, &cfg.OwnerEmail)
	}
	if v, ok := raw["plan"]; ok {
		_ = json.Unmarshal(v, &cfg.Plan)
	}
	return &cfg, nil
}
